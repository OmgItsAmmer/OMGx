...
import 'package:ecommerce_dashboard/repositories/customer/customer_repository.dart';
...
class CustomerController extends GetxController {
  ...
  int customerId = -1;
  GlobalKey<FormState> addCustomerKey = GlobalKey<FormState>();
  ...
  Future<void> updateCustomer(int customerId) async {
    try {
      ...
      final json =
          customerModel.toJson(isUpdate: false); // we need id for updating
  ...
  Future<void> insertCustomer() async {
    try {
      ...
      final json =
          customerModel.toJson(isUpdate: true); // we need id for updating
  ...
  void setCustomerDetail(CustomerModel customer) {
    try {
      ...
      final matchingAddress =
          AddressController.instance.allCustomerAddresses.firstWhere(
        (address) => address.customerId == customer.customerId,
        orElse: () =>
            AddressModel.empty(), // Return null if no matching address is found
      );
  ...
  Future<void> prepareCustomerDetails(dynamic customerId) async {
    try {
      // Loading state
      isLoading.value = true;
