...
class SalesmanRepository extends GetxController {
  ...
  Future<int?> saveOrUpdateSalesmanRepo(Map<String, dynamic> json) async {
    try {
      ...
      TLoaders.errorSnackBar(title: 'Salesman Repo', message: e.message);
      rethrow;
    } catch (e) {
      TLoaders.errorSnackBar(title: 'Salesman Repo', message: e.toString());
      rethrow;
    }
  }
    ...
    try {
      ...
      if (kDebugMode) {
        ...
        TLoaders.errorSnackBar(
            title: 'insertSalesmanInTable', message: e.message);
      }
      rethrow;
    } catch (e) {
      if (kDebugMode) {
        print(e);
        TLoaders.errorSnackBar(
            title: 'insertSalesmanInTable', message: e.toString());
      }
      rethrow;
    }
  }
  ...
  Future<void> updateSalesman(Map<String, dynamic> json) async {
    try {
      ...
      if (kDebugMode) {
        print(e);
        TLoaders.errorSnackBar(title: 'updateSalesman', message: e.message);
      }
      rethrow;
    } catch (e) {
      if (kDebugMode) {
        print(e);
        TLoaders.errorSnackBar(title: 'updateSalesman', message: e.toString());
      }
      rethrow;
    }
  }
  ...
  Future<void> deleteSalesmanFromTable(int customerId) async {
    try {
      ...
      TLoaders.successSnackBar(
          title: "Success", message: "Salesman deleted successfully");
    } catch (e) {
      if (kDebugMode) {
        TLoaders.errorSnackBar(
            title: 'deleteSalesmanFromTable', message: e.toString());
