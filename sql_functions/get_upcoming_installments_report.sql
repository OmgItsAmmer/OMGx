CREATE OR REPLACE FUNCTION get_upcoming_installments_report(
  days_ahead INTEGER
)
RETURNS SETOF json
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'due_date', TO_CHAR(COALESCE(inst.due_date, CURRENT_DATE), 'YYYY-MM-DD'),
    ...
    'days_until_due', COALESCE((inst.due_date::date - CURRENT_DATE)::int, 0)
  )
  ...
  WHERE inst.due_date IS NOT NULL
    ...
    AND inst.due_date::date <= (CURRENT_DATE + INTERVAL '1 day' * days_ahead)
